...
#define BINS 16//cambio 16 por 32
using namespace std;
...
{
	double limiar = 0.4;  //0.4
	int k_aux = 0;
	while(k_aux == 0 && limiar > -1)
	{
		cout<<"Num features: "<<(int)features.size()-1<<endl;
		for(int i=0; i<(int)features.size()-1; i++)
		{
			cout<<"euclidianDist: "<<euclidianDist(features[i+1].getHistogramNorm(), features[i].getHistogramNorm())<<endl;
...
int ClusteringCPU::findNearestCluster(HistogramCPU hist)
{
	...
	cout<<"numClustersK: "<<(int)clusters.size()<<endl;
...
void ClusteringCPU::removeSimilarKeyframes()
{
	...
	cout<<"funcion eliminar:"<<keyframes.size()<<endl;
	for(int i=0; i<(int)keyframes.size()-1; i++)
	{
		for(int j=i+1; j<(int)keyframes.size(); j++)
		// int j=i+1;
		// while(j<keyframes.size()) // cambio (int)keyframes.size() por keyframes.size()
		{
			if(euclidianDist(keyframes[i].getHistogramNorm(), keyframes[j].getHistogramNorm())<0.1) //0.3
			{
